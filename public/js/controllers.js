...
function Stuck() {
    ...
    if (onboard[currpawn] == 0||currPos+num>44) {
        if (DontHaveOtherFree()||currPos+num>44) {
            ...
            dice.style.backgroundImage = "../img/board.gif";
            window.setTimeout(changePlayer, 1000);
        }
    }
}
//set the turns for players
...
//########## Displacement one Tile at a time
function stepDown() {
    var doc = document.getElementById(currcolor + "pawn"+NumOfPaw); //selectes an specific pawn to move
    var curr = Number(doc.style.top.replace(/[a-z]/g, '')); //gets the y coordinate of the pwan
    doc.style.top = (curr+step)+'px'; //updates the y coordinate of the pawn 
    currPos++; //relative number of tile
}
function stepUp() {
...
//Defines the path for every one of the pawns 
var stepsRed = [];
var stepsYellow = [];
var stepsBlue =[];
var stepsGreen =[];
...
function randomNum() {
    if (!clicked) {
        num = Math.floor((Math.random() * 6) + 1);; //random number
        //die animation
        startAnimation(num);
        clicked = true;
    }
            ...
            if (text.innerText == currcolor) { //**********************
                if (onboard[currpawn] === 1 || num === 6) {
                    if (onboard[currpawn] === 0) {
                        ...
                        var curr = Number(doc.style.left.replace(/[a-z]/g, ''));
                        switch (Color) {
                           // 570-left & 22-top were added
                           case "red":
                            doc.style.left = 888 + 'px';
                            doc.style.top = 50 + "px";
                            break;

                        case "yellow":
                            doc.style.left = 789 + 'px';
                            doc.style.top = 545 + "px";
                            break;

                        case "blue":
                            doc.style.left = 1086 + 'px';
                            doc.style.top = 347 + "px";
                            break;

                        case "green":
                            doc.style.left = 591 + 'px';
                            doc.style.top = 248 + "px";
                            break;
                        }
                        onboard[currpawn] = 1;
                    }
